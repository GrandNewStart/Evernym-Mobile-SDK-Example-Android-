...
object SDKInitialization {
    ...
    private fun getProvisionToken(completionHandler: (String?) -> Unit) {
        ...
        val requestBody =
            json.toString().toRequestBody("application/json;charset=utf8".toMediaType())
    ...
    private fun provisionCloudAgentAndInitializeSdk(completionHandler: (String?) -> Unit) {
        ...
        getProvisionToken { token ->
            ...
            UtilsApi.vcxAgentProvisionWithTokenAsync(config, token)
                .whenCompleteAsync { oneTimeInfo, error ->
                    (error as? VcxException)?.let {
                        it.print("SDKInitialization", "provisionCloudAgentAndInitializeSdk")
                        App.shared.isLoading.postValue(false)
    ...
    private fun initialize(completionHandler: (String?) -> Unit) {
        ...
        VcxApi.vcxInitWithConfig(config).whenCompleteAsync { _, error ->
            (error as? VcxException)?.let {
                it.print("SDKInitialization", "initialize")
                SDKStorage.removeVcxConfig()
