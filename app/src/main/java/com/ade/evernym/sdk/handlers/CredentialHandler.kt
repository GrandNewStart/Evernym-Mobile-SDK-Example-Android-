...
object CredentialHandler {
    ...
    fun getCredential(
        ...
        connection.deserialize { connectionHandle ->
            ...
            CredentialApi.credentialCreateWithMsgid(
                ...
                val credentialOffer = JSONObject(result.offer).getJSONObjectOptional("credential_offer")
                if (credentialOffer == null) {
                    Log.e("CredentialHandler","getCredential: (3): failed to get 'credential_offer")
                ...
                if (attrs == null) {
                    Log.e("CredentialHandler","getCredential: (6): failed to get 'credential_attrs")
                ...
                CredentialApi.credentialSerialize(result.credential_handle).whenComplete { serialized, error2 ->
                    (error2 as? VcxException)?.let {
    ...
    fun acceptCredential(credential: DIDCredential, completionHandler: (credential: DIDCredential?, String?)->Unit) {
        val connection = DIDConnection.getById(credential.connectionId)
        if (connection == null) {
            Log.e("CredentialHandler","acceptCredential: (1) cannot find connection by id ${credential.connectionId}")
            completionHandler(null, "cannot find connection by id ${credential.connectionId}")
            return
        }
        connection.deserialize { connectionHandle ->
            if (connectionHandle == null) {
                Log.e("CredentialHandler","acceptCredential: (2) failed to deserialize connection")
                completionHandler(null, "failed to deserialize connection")
                return@deserialize
            }
            credential.deserialize { credentialHandle ->
                if (credentialHandle == null) {
                    Log.e("CredentialHandler","acceptCredential: (3) failed to deserialize credential")
                ...
                CredentialApi.credentialSendRequest(
                    ...
                    (error1 as? VcxException)?.let {
                        it.print("CredentialHandler", "acceptCredential: (4)")
                        completionHandler(null, it.localizedMessage)
                        return@whenComplete
                    }
                    CredentialApi.credentialSerialize(credentialHandle).whenComplete { serialized, error2 ->
                        ...
                        awaitCredentialReceived(credential) { error3 ->
                            error3?.let {
                                Log.e("CredentialHandler", "acceptCredential: (6) $it")
    ...
    private fun awaitCredentialReceived(credential: DIDCredential, completionHandler: (String?) -> Unit) {
        credential.deserialize { credentialHandle ->
            if (credentialHandle == null) {
                Log.e("CredentialHandler", "awaitCredentialReceived: (1) failed to deserialize credential")
                completionHandler("failed to deserialize credential")
                return@deserialize
            }
            while(true) {
                try {
                    val state = CredentialApi.credentialUpdateState(credentialHandle).get()
                    Log.d("CredentialHandler", "awaitCredentialReceived: state($state)")
                    if (state == 4) {
                        CredentialApi.credentialSerialize(credentialHandle).whenComplete { serialized, error2 ->
                            (error2 as? VcxException)?.let {
                                it.print("CredentialHandler", "awaitCredentialReceived: (2)")
                            ...
                            credential.getReferent { data ->
                                if (data == null) {
                                    Log.e("CredentialHandler", "awaitCredentialReceived: (3) failed to get referent")
                    ...
                    e.print("CredentialHandler", "awaitCredentialReceived: (4)")
                }
            }
        }
    }
    ...
    fun rejectCredential(credential: DIDCredential, completionHandler: (String?) -> Unit) {
        ...
        val connection = DIDConnection.getById(credential.connectionId)
        if (connection == null) {
            Log.e("CredentialHandler", "rejectCredential: (1) cannot find connection by id ${credential.connectionId}")
        ...
        connection.deserialize { connectionHandle ->
            ...
            credential.deserialize { credentialHandle ->
                if (credentialHandle == null) {
                    Log.e("CredentialHandler", "rejectCredential: (3) failed to deserialize credential")
                    completionHandler("failed to deserialize credential")
                    return@deserialize
                }
                CredentialApi.credentialReject(credentialHandle, connectionHandle, "").whenComplete { _, error ->
